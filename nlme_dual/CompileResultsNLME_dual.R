...
# which rows to use for Cmax/Cmin, Cmax may be 2nd or 3rd sample
  ...
  for(this_vwt in 0:1){
    for(this_comp in 0:2){
      for(this_eta in 0:5){
        ...
        write.csv(Results,file.path(home_dir,"NLMEDualResults.csv"),quote= FALSE, row.names = FALSE)
      }
    }
  }
}
