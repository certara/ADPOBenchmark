...
int GetNThetaOld(){	// get number of theta minus possibly 1 sigma to estimate
	int _n = 9 + 1; // number of fixefs + number of epsilons
...
void InitFixefErrorEstimate(){
	...
	_fixef[7] = __K13_init; // set K13
	_fixef[8] = __K31_init; // set K31
	_fixef[9] = 0.5; // stdev 0
}
...
void GetFixefErrorLimits(double* _low, double* _high){
	...
	_high[6] = __K21_high;
	_low[7] = __K13_low; // set K13
	_high[7] = __K13_high;
	_low[8] = __K31_low; // set K31
	_high[8] = __K31_high;
	_low[9] = 0; // stdev 0
	_high[9] = 1e100; // stdev 0
}
...
void Deriv(long* _pN, double* _pt, double zzY[], double zzR[], double zzIRate[]){
	...
	zzR_Aa = ( -(Ka) * Aa);
	zzR_Aa += _Aa_irate;
	zzR_A1 = ((((((Ka * Aa) + (K21 * A2)) - (K12 * A1)) + (K31 * A3)) - (K12 * A2)) - ((Vmax * C) / (Km + C)));
	zzR_A1 += _A1_irate;
	...
	// 0.0
	// 0.0
	// ( -(K12) - (((((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvVmax * exp(nVmax)) / (tvV * exp(nKm)))) - (((tvVmax * exp(nVmax)) * (A1 / (tvV * exp(nKm)))) / (tvV * exp(nKm)))) / (((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))))))  is not constant
	// K12
	// K13
	// 0.0
	// (K21 - K12)
...
void Jacobian(int* pN, double* pt, double zzY[], double zzIRate[], int* pML, int* pMU, double* _jacobian, int* pNRJ){
	...
	_jacobian[1 + _nrj*0] = (tvKa * __temp[0]);
		// w.r.t. A1
	_jacobian[1 + _nrj*1] = ( -(K12) - (((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvVmax * __temp[3]) / (tvV * __temp[2]))) - (((tvVmax * __temp[3]) * (A1 / (tvV * __temp[2]))) / (tvV * __temp[2]))) / (((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))))));
		// w.r.t. A2
...
void GetSaemThetaInfo(int* iaCColToITheta, int* baCColIsLog){
	int _jCTheta = 0;
	int _nTheta = 0;
	int _iaIFixefErrToITheta[9+1];
	int _i;
	for (_i = 0; _i < 9+1; _i++) _iaIFixefErrToITheta[_i] = -1;
	...
	{
		iaCColToITheta[_jCTheta] = _iaIFixefErrToITheta[7]; // K13
		baCColIsLog[_jCTheta] = 0;
		_jCTheta++;
	}

	{
		iaCColToITheta[_jCTheta] = _iaIFixefErrToITheta[8]; // K31
		baCColIsLog[_jCTheta] = 0;
		_jCTheta++;
	}
...
void GradientDeriv(long* _pN, double* _pt, double zzY[], double zzR[], double zzIRate[])
{
	...
	zzR_A3 += _A3_irate;
	zzR[4] = 0; // extra null equation
	zzR__A1_nVmax = (((((Ka * _Aa_nVmax) + (( -(K12) - ((((Km + (A1 / V)) * (Vmax / V)) - ((Vmax * (A1 / V)) / V)) / ((Km + (A1 / V)) * (Km + (A1 / V))))) * _A1_nVmax)) + ((K21 - K12) * _A2_nVmax)) + (K31 * _A3_nVmax)) +  -((((Km + (A1 / V)) * ((A1 / V) * Vmax)) / ((Km + (A1 / V)) * (Km + (A1 / V))))));
	zzR__A2_nVmax = ((K12 * _A1_nVmax) + ( -(K21) * _A2_nVmax));
	zzR__A3_nVmax = ((K13 * _A1_nVmax) + ( -(K31) * _A3_nVmax));
	zzR__A1_nKm = (((((Ka * _Aa_nKm) + (( -(K12) - ((((Km + (A1 / V)) * (Vmax / V)) - ((Vmax * (A1 / V)) / V)) / ((Km + (A1 / V)) * (Km + (A1 / V))))) * _A1_nKm)) + ((K21 - K12) * _A2_nKm)) + (K31 * _A3_nKm)) +  -(((((Km + (A1 / V)) * (Vmax * ( -((A1 * V)) / (V * V)))) - ((Vmax * (A1 / V)) * ( -((A1 * V)) / (V * V)))) / ((Km + (A1 / V)) * (Km + (A1 / V))))));
	zzR__A2_nKm = ((K12 * _A1_nKm) + ( -(K21) * _A2_nKm));
	zzR__A3_nKm = ((K13 * _A1_nKm) + ( -(K31) * _A3_nKm));
	zzR__A1_nV = (((((Ka * _Aa_nV) + (( -(K12) - ((((Km + (A1 / V)) * (Vmax / V)) - ((Vmax * (A1 / V)) / V)) / ((Km + (A1 / V)) * (Km + (A1 / V))))) * _A1_nV)) + ((K21 - K12) * _A2_nV)) + (K31 * _A3_nV)) +  -(( -(((Vmax * (A1 / V)) * Km)) / ((Km + (A1 / V)) * (Km + (A1 / V))))));
	zzR__A2_nV = ((K12 * _A1_nV) + ( -(K21) * _A2_nV));
	zzR__A3_nV = ((K13 * _A1_nV) + ( -(K31) * _A3_nV));
	zzR__Aa_nKa = (( -(Ka) * _Aa_nKa) + (Aa *  -(Ka)));
	zzR__A1_nKa = (((((Ka * _Aa_nKa) + (( -(K12) - ((((Km + (A1 / V)) * (Vmax / V)) - ((Vmax * (A1 / V)) / V)) / ((Km + (A1 / V)) * (Km + (A1 / V))))) * _A1_nKa)) + ((K21 - K12) * _A2_nKa)) + (K31 * _A3_nKa)) + (Aa * Ka));
	...
	//[0,1] 0.0
	//[1,1] ( -(K12) - (((((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvVmax * exp(nVmax)) / (tvV * exp(nKm)))) - (((tvVmax * exp(nVmax)) * (A1 / (tvV * exp(nKm)))) / (tvV * exp(nKm)))) / (((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))))))  is not constant
	//[2,1] K12
	//[3,1] K13
	...
	//[6,1] 0.0
	//[7,1] 0.0
	//[8,1] ((_A1_nKm *  -(((((((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm))))) * ((((tvVmax * exp(nVmax)) / (tvV * exp(nKm))) / (tvV * exp(nKm))) - (((tvV * exp(nKm)) * ((tvVmax * exp(nVmax)) / (tvV * exp(nKm)))) / ((tvV * exp(nKm)) * (tvV * exp(nKm)))))) - (((((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvVmax * exp(nVmax)) / (tvV * exp(nKm)))) - (((tvVmax * exp(nVmax)) * (A1 / (tvV * exp(nKm)))) / (tvV * exp(nKm)))) * ((((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) / (tvV * exp(nKm))) + (((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) / (tvV * exp(nKm)))))) / ((((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm))))) * (((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm))))))))) +  -(((((((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm))))) * (((((tvVmax * exp(nVmax)) * ( -((A1 * (tvV * exp(nKm)))) / ((tvV * exp(nKm)) * (tvV * exp(nKm))))) / (tvV * exp(nKm))) + (((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvVmax * exp(nVmax)) * ((((tvV * exp(nKm)) * (tvV * exp(nKm))) *  -((tvV * exp(nKm)))) / (((tvV * exp(nKm)) * (tvV * exp(nKm))) * ((tvV * exp(nKm)) * (tvV * exp(nKm)))))))) - ((( -((A1 * (tvV * exp(nKm)))) / ((tvV * exp(nKm)) * (tvV * exp(nKm)))) * ((tvVmax * exp(nVmax)) / (tvV * exp(nKm)))) + (((tvVmax * exp(nVmax)) * (A1 / (tvV * exp(nKm)))) * ((((tvV * exp(nKm)) * (tvV * exp(nKm))) *  -((tvV * exp(nKm)))) / (((tvV * exp(nKm)) * (tvV * exp(nKm))) * ((tvV * exp(nKm)) * (tvV * exp(nKm))))))))) - (((((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvVmax * exp(nVmax)) * ( -((A1 * (tvV * exp(nKm)))) / ((tvV * exp(nKm)) * (tvV * exp(nKm)))))) - (((tvVmax * exp(nVmax)) * (A1 / (tvV * exp(nKm)))) * ( -((A1 * (tvV * exp(nKm)))) / ((tvV * exp(nKm)) * (tvV * exp(nKm)))))) * ((((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) / (tvV * exp(nKm))) + (((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) / (tvV * exp(nKm)))))) / ((((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm))))) * (((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))))))))  is not constant
	//[9,1] 0.0
	//[10,1] 0.0
	//[11,1] ((_A1_nV *  -(((((((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm))))) * ((((tvVmax * exp(nVmax)) / (tvV * exp(nKm))) / (tvV * exp(nKm))) - (((tvV * exp(nKm)) * ((tvVmax * exp(nVmax)) / (tvV * exp(nKm)))) / ((tvV * exp(nKm)) * (tvV * exp(nKm)))))) - (((((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvVmax * exp(nVmax)) / (tvV * exp(nKm)))) - (((tvVmax * exp(nVmax)) * (A1 / (tvV * exp(nKm)))) / (tvV * exp(nKm)))) * ((((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) / (tvV * exp(nKm))) + (((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) / (tvV * exp(nKm)))))) / ((((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm))))) * (((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm))))))))) +  -(((((((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm))))) *  -(((tvKm * exp(nV)) * ((tvVmax * exp(nVmax)) / (tvV * exp(nKm)))))) - ( -((((tvVmax * exp(nVmax)) * (A1 / (tvV * exp(nKm)))) * (tvKm * exp(nV)))) * ((((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) / (tvV * exp(nKm))) + (((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) / (tvV * exp(nKm)))))) / ((((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm))))) * (((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))))))))  is not constant
	//[12,1] 0.0
	//[13,1] 0.0
	//[14,1] 0.0
	//[15,1] (_A1_nKa *  -(((((((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm))))) * ((((tvVmax * exp(nVmax)) / (tvV * exp(nKm))) / (tvV * exp(nKm))) - (((tvV * exp(nKm)) * ((tvVmax * exp(nVmax)) / (tvV * exp(nKm)))) / ((tvV * exp(nKm)) * (tvV * exp(nKm)))))) - (((((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvVmax * exp(nVmax)) / (tvV * exp(nKm)))) - (((tvVmax * exp(nVmax)) * (A1 / (tvV * exp(nKm)))) / (tvV * exp(nKm)))) * ((((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) / (tvV * exp(nKm))) + (((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) / (tvV * exp(nKm)))))) / ((((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm))))) * (((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))))))))  is not constant
	//[16,1] 0.0
	//[17,1] 0.0
	...
	//[2,5] 0.0
	//[3,5] 0.0
	//[5,5] ( -(K12) - (((((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvVmax * exp(nVmax)) / (tvV * exp(nKm)))) - (((tvVmax * exp(nVmax)) * (A1 / (tvV * exp(nKm)))) / (tvV * exp(nKm)))) / (((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))))))  is not constant
	//[6,5] K12
	//[7,5] K13
	//[8,5] 0.0
	//[9,5] 0.0
	...
	//[6,8] 0.0
	//[7,8] 0.0
	//[8,8] ( -(K12) - (((((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvVmax * exp(nVmax)) / (tvV * exp(nKm)))) - (((tvVmax * exp(nVmax)) * (A1 / (tvV * exp(nKm)))) / (tvV * exp(nKm)))) / (((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))))))  is not constant
	//[9,8] K12
	//[10,8] K13
	//[11,8] 0.0
	//[12,8] 0.0
	...
	//[9,11] 0.0
	//[10,11] 0.0
	//[11,11] ( -(K12) - (((((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvVmax * exp(nVmax)) / (tvV * exp(nKm)))) - (((tvVmax * exp(nVmax)) * (A1 / (tvV * exp(nKm)))) / (tvV * exp(nKm)))) / (((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))))))  is not constant
	//[12,11] K12
	//[13,11] K13
	//[14,11] 0.0
	//[15,11] 0.0
	...
	//[13,15] 0.0
	//[14,15] 0.0
	//[15,15] ( -(K12) - (((((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvVmax * exp(nVmax)) / (tvV * exp(nKm)))) - (((tvVmax * exp(nVmax)) * (A1 / (tvV * exp(nKm)))) / (tvV * exp(nKm)))) / (((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))) * ((tvKm * exp(nV)) + (A1 / (tvV * exp(nKm)))))))  is not constant
	//[16,15] K12
	//[17,15] K13
...
void GradientJacobian(int* pN, double* pt, double zzY[], double zzIRate[], int* pML, int* pMU, double* _gjacobian, int* pNRJ){
	...
	_gjacobian[1 + _nrj*0] = (tvKa * __temp[0]);
		// w.r.t. A1
	_gjacobian[1 + _nrj*1] = ( -(K12) - (((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvVmax * __temp[3]) / (tvV * __temp[2]))) - (((tvVmax * __temp[3]) * (A1 / (tvV * __temp[2]))) / (tvV * __temp[2]))) / (((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))))));
		// w.r.t. A2
	...
	// rate terms for _A1_nVmax
		// w.r.t. A1
	_gjacobian[5 + _nrj*1] = ((_A1_nVmax *  -(((((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvKm * __temp[1]) + (A1 / (tvV * __temp[2])))) * ((((tvVmax * __temp[3]) / (tvV * __temp[2])) / (tvV * __temp[2])) - (((tvV * __temp[2]) * ((tvVmax * __temp[3]) / (tvV * __temp[2]))) / ((tvV * __temp[2]) * (tvV * __temp[2]))))) - (((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvVmax * __temp[3]) / (tvV * __temp[2]))) - (((tvVmax * __temp[3]) * (A1 / (tvV * __temp[2]))) / (tvV * __temp[2]))) * ((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) / (tvV * __temp[2])) + (((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) / (tvV * __temp[2]))))) / ((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvKm * __temp[1]) + (A1 / (tvV * __temp[2])))) * (((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvKm * __temp[1]) + (A1 / (tvV * __temp[2])))))))) +  -(((((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvKm * __temp[1]) + (A1 / (tvV * __temp[2])))) * ((((A1 / (tvV * __temp[2])) * (tvVmax * __temp[3])) / (tvV * __temp[2])) + (((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvVmax * __temp[3]) / (tvV * __temp[2]))))) - ((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((A1 / (tvV * __temp[2])) * (tvVmax * __temp[3]))) * ((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) / (tvV * __temp[2])) + (((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) / (tvV * __temp[2]))))) / ((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvKm * __temp[1]) + (A1 / (tvV * __temp[2])))) * (((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))))))));
		// w.r.t. _A1_nVmax
	_gjacobian[5 + _nrj*5] = ( -(K12) - (((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvVmax * __temp[3]) / (tvV * __temp[2]))) - (((tvVmax * __temp[3]) * (A1 / (tvV * __temp[2]))) / (tvV * __temp[2]))) / (((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))))));
		// w.r.t. _A2_nVmax
	...
	// rate terms for _A1_nKm
		// w.r.t. A1
	_gjacobian[8 + _nrj*1] = ((_A1_nKm *  -(((((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvKm * __temp[1]) + (A1 / (tvV * __temp[2])))) * ((((tvVmax * __temp[3]) / (tvV * __temp[2])) / (tvV * __temp[2])) - (((tvV * __temp[2]) * ((tvVmax * __temp[3]) / (tvV * __temp[2]))) / ((tvV * __temp[2]) * (tvV * __temp[2]))))) - (((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvVmax * __temp[3]) / (tvV * __temp[2]))) - (((tvVmax * __temp[3]) * (A1 / (tvV * __temp[2]))) / (tvV * __temp[2]))) * ((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) / (tvV * __temp[2])) + (((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) / (tvV * __temp[2]))))) / ((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvKm * __temp[1]) + (A1 / (tvV * __temp[2])))) * (((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvKm * __temp[1]) + (A1 / (tvV * __temp[2])))))))) +  -(((((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvKm * __temp[1]) + (A1 / (tvV * __temp[2])))) * (((((tvVmax * __temp[3]) * ( -((A1 * (tvV * __temp[2]))) / ((tvV * __temp[2]) * (tvV * __temp[2])))) / (tvV * __temp[2])) + (((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvVmax * __temp[3]) * ((((tvV * __temp[2]) * (tvV * __temp[2])) *  -((tvV * __temp[2]))) / (((tvV * __temp[2]) * (tvV * __temp[2])) * ((tvV * __temp[2]) * (tvV * __temp[2]))))))) - ((( -((A1 * (tvV * __temp[2]))) / ((tvV * __temp[2]) * (tvV * __temp[2]))) * ((tvVmax * __temp[3]) / (tvV * __temp[2]))) + (((tvVmax * __temp[3]) * (A1 / (tvV * __temp[2]))) * ((((tvV * __temp[2]) * (tvV * __temp[2])) *  -((tvV * __temp[2]))) / (((tvV * __temp[2]) * (tvV * __temp[2])) * ((tvV * __temp[2]) * (tvV * __temp[2])))))))) - (((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvVmax * __temp[3]) * ( -((A1 * (tvV * __temp[2]))) / ((tvV * __temp[2]) * (tvV * __temp[2]))))) - (((tvVmax * __temp[3]) * (A1 / (tvV * __temp[2]))) * ( -((A1 * (tvV * __temp[2]))) / ((tvV * __temp[2]) * (tvV * __temp[2]))))) * ((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) / (tvV * __temp[2])) + (((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) / (tvV * __temp[2]))))) / ((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvKm * __temp[1]) + (A1 / (tvV * __temp[2])))) * (((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))))))));
		// w.r.t. _A1_nKm
	_gjacobian[8 + _nrj*8] = ( -(K12) - (((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvVmax * __temp[3]) / (tvV * __temp[2]))) - (((tvVmax * __temp[3]) * (A1 / (tvV * __temp[2]))) / (tvV * __temp[2]))) / (((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))))));
		// w.r.t. _A2_nKm
	...
	// rate terms for _A1_nV
		// w.r.t. A1
	_gjacobian[11 + _nrj*1] = ((_A1_nV *  -(((((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvKm * __temp[1]) + (A1 / (tvV * __temp[2])))) * ((((tvVmax * __temp[3]) / (tvV * __temp[2])) / (tvV * __temp[2])) - (((tvV * __temp[2]) * ((tvVmax * __temp[3]) / (tvV * __temp[2]))) / ((tvV * __temp[2]) * (tvV * __temp[2]))))) - (((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvVmax * __temp[3]) / (tvV * __temp[2]))) - (((tvVmax * __temp[3]) * (A1 / (tvV * __temp[2]))) / (tvV * __temp[2]))) * ((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) / (tvV * __temp[2])) + (((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) / (tvV * __temp[2]))))) / ((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvKm * __temp[1]) + (A1 / (tvV * __temp[2])))) * (((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvKm * __temp[1]) + (A1 / (tvV * __temp[2])))))))) +  -(((((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvKm * __temp[1]) + (A1 / (tvV * __temp[2])))) *  -(((tvKm * __temp[1]) * ((tvVmax * __temp[3]) / (tvV * __temp[2]))))) - ( -((((tvVmax * __temp[3]) * (A1 / (tvV * __temp[2]))) * (tvKm * __temp[1]))) * ((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) / (tvV * __temp[2])) + (((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) / (tvV * __temp[2]))))) / ((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvKm * __temp[1]) + (A1 / (tvV * __temp[2])))) * (((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))))))));
		// w.r.t. _A1_nV
	_gjacobian[11 + _nrj*11] = ( -(K12) - (((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvVmax * __temp[3]) / (tvV * __temp[2]))) - (((tvVmax * __temp[3]) * (A1 / (tvV * __temp[2]))) / (tvV * __temp[2]))) / (((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))))));
		// w.r.t. _A2_nV
	...
	_gjacobian[15 + _nrj*0] = (tvKa * __temp[0]);
		// w.r.t. A1
	_gjacobian[15 + _nrj*1] = (_A1_nKa *  -(((((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvKm * __temp[1]) + (A1 / (tvV * __temp[2])))) * ((((tvVmax * __temp[3]) / (tvV * __temp[2])) / (tvV * __temp[2])) - (((tvV * __temp[2]) * ((tvVmax * __temp[3]) / (tvV * __temp[2]))) / ((tvV * __temp[2]) * (tvV * __temp[2]))))) - (((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvVmax * __temp[3]) / (tvV * __temp[2]))) - (((tvVmax * __temp[3]) * (A1 / (tvV * __temp[2]))) / (tvV * __temp[2]))) * ((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) / (tvV * __temp[2])) + (((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) / (tvV * __temp[2]))))) / ((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvKm * __temp[1]) + (A1 / (tvV * __temp[2])))) * (((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))))))));
		// w.r.t. _Aa_nKa
	_gjacobian[15 + _nrj*14] = (tvKa * __temp[0]);
		// w.r.t. _A1_nKa
	_gjacobian[15 + _nrj*15] = ( -(K12) - (((((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvVmax * __temp[3]) / (tvV * __temp[2]))) - (((tvVmax * __temp[3]) * (A1 / (tvV * __temp[2]))) / (tvV * __temp[2]))) / (((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))) * ((tvKm * __temp[1]) + (A1 / (tvV * __temp[2]))))));
		// w.r.t. _A2_nKa
    ...
    if (_nm[0]=='C' && strcmp(_nm, "CObs")==0){
        ...
        if (FALSE && ALLOWLOGTRANSFORM && bEnableLogTransform){
            *_pPred = logfl((C + (_eps * sqrt((1.0 + (pow(C, 2.0) * pow((CMultStdev / sigma()), 2.0)))))));
            _eps = _pcb->GetNormal() * _fixef[9]; // get a N(0,stdev)
