...
#define     A3       zzY[3]
#define zzR_A3       zzR[3]
#define    _A3_irate zzIRate[3]

 // zzY[4] is extra integrator for infusions

int _i_GAMMA = -1;	// index of GAMMA in _fixef
static double ___GAMMA = 0;		// value of GAMMA if it is disabled
#define GAMMA (*(_i_GAMMA >= 0 ? &_fixef[_i_GAMMA] : &___GAMMA))
...
int _i_K13 = -1;	// index of K13 in _fixef
static double ___K13 = 0;		// value of K13 if it is disabled
#define K13 (*(_i_K13 >= 0 ? &_fixef[_i_K13] : &___K13))
int _i_K31 = -1;	// index of K31 in _fixef
static double ___K31 = 0;		// value of K31 if it is disabled
#define K31 (*(_i_K31 >= 0 ? &_fixef[_i_K31] : &___K31))
...
int GetNThetaOld(){	// get number of theta minus possibly 1 sigma to estimate
	int _n = 10 + 1; // number of fixefs + number of epsilons
...
static double __GAMMA_low  = 0.0001000000000000000048;
static double __GAMMA_init = 1.600000000000000089;
static double __GAMMA_high = 3.0;
...
static double __K13_low  = 0.0001000000000000000048;
static double __K13_init = 0.1000000000000000056;
static double __K13_high = 10.0;
static double __K31_low  = 0.0001000000000000000048;
static double __K31_init = 0.05000000000000000278;
static double __K31_high = 10.0;
void RestoreFixefLowInitHighFromModel(){
	__GAMMA_low  = 0.0001000000000000000048;
	__GAMMA_init = 1.600000000000000089;
	__GAMMA_high = 3.0;
	...
	__K21_init = 3.0;
	__K21_high = 10.0;
	__K13_low  = 0.0001000000000000000048;
	__K13_init = 0.1000000000000000056;
	__K13_high = 10.0;
	__K31_low  = 0.0001000000000000000048;
	__K31_init = 0.05000000000000000278;
	__K31_high = 10.0;
}
void SetFixefLowInitHigh(const char* _nm, double* _fixefLow, double* _fixefInit, double* _fixefHigh){
    if (strcmp(_nm, "GAMMA")==0){
        __GAMMA_low  = *_fixefLow;
        __GAMMA_init = *_fixefInit;
        __GAMMA_high = *_fixefHigh;
        return;
	}
    ...
    if (strcmp(_nm, "K13")==0){
        __K13_low  = *_fixefLow;
        __K13_init = *_fixefInit;
        __K13_high = *_fixefHigh;
        return;
	}
    if (strcmp(_nm, "K31")==0){
        __K31_low  = *_fixefLow;
        __K31_init = *_fixefInit;
        __K31_high = *_fixefHigh;
        return;
	}
}
void GetFixefLowInitHigh(const char* _nm, double* _fixefLow, double* _fixefInit, double* _fixefHigh){
    ...
    if (strcmp(_nm, "GAMMA")==0){
        *_fixefLow  = __GAMMA_low;
        *_fixefInit = __GAMMA_init;
        *_fixefHigh = __GAMMA_high;
        return;
    }
    if (strcmp(_nm, "CMultStdev")==0){
    ...
    if (strcmp(_nm, "K13")==0){
        *_fixefLow  = __K13_low;
        *_fixefInit = __K13_init;
        *_fixefHigh = __K13_high;
        return;
    }
    if (strcmp(_nm, "K31")==0){
        *_fixefLow  = __K31_low;
        *_fixefInit = __K31_init;
        *_fixefHigh = __K31_high;
        return;
	}
}
void GetThetaLimits(double* _fixefLow, double* _fixefHigh){
	int _n = 0;
	_fixefLow[_n] = __GAMMA_low; // set GAMMA
	_fixefHigh[_n] = __GAMMA_high;
	_n++;
	...
	_n++;
	_fixefLow[_n] = __K13_low; // set K13
	_fixefHigh[_n] = __K13_high;
	_n++;
	_fixefLow[_n] = __K31_low; // set K31
	_fixefHigh[_n] = __K31_high;
	_n++;
...
void InitFixefErrorEstimate(){
	double* _fixef = _args[0];
	_fixef[0] = __GAMMA_init; // set GAMMA
	_fixef[1] = __CMultStdev_init; // set CMultStdev
	_fixef[2] = __tvVmax_init; // set tvVmax
	_fixef[3] = __tvKm_init; // set tvKm
	_fixef[4] = __tvV_init; // set tvV
	_fixef[5] = __tvKa_init; // set tvKa
	_fixef[6] = __K12_init; // set K12
	_fixef[7] = __K21_init; // set K21
	_fixef[8] = __K13_init; // set K13
	_fixef[9] = __K31_init; // set K31
	_fixef[10] = 0.5; // stdev 0
}
...
void GetFixefErrorLimits(double* _low, double* _high){
	_low[0] = __GAMMA_low; // set GAMMA
	_high[0] = __GAMMA_high;
	_low[1] = __CMultStdev_low; // set CMultStdev
	_high[1] = __CMultStdev_high;
	_low[2] = __tvVmax_low; // set tvVmax
	_high[2] = __tvVmax_high;
	_low[3] = __tvKm_low; // set tvKm
	_high[3] = __tvKm_high;
	_low[4] = __tvV_low; // set tvV
	_high[4] = __tvV_high;
	_low[5] = __tvKa_low; // set tvKa
	_high[5] = __tvKa_high;
	_low[6] = __K12_low; // set K12
	_high[6] = __K12_high;
	_low[7] = __K21_low; // set K21
	_high[7] = __K21_high;
	_low[8] = __K13_low; // set K13
	_high[8] = __K13_high;
	_low[9] = __K31_low; // set K31
	_high[9] = __K31_high;
	_low[10] = 0; // stdev 0
	_high[10] = 1e100; // stdev 0
}
...
void GetInitialOmega5(double* _omat){
	int _i = 0;
	double _temp = 0;
	for (_i=0; _i<16; _i++) _omat[_i] = 0;
	_omat[0+0+0*4] = 0.3099999999999999978;
	_omat[0+1+1*4] = 0.3099999999999999978;
	_omat[0+2+2*4] = 0.3099999999999999978;
	_omat[0+3+3*4] = 0.3099999999999999978;
}
...
void TruncateOmega5(double* _omat){
	...
	_temp = _mtemp[0+0+0*4];
	_omat[0+0+0*4] = _temp;
	_temp = _mtemp[0+1+1*4];
	_omat[0+1+1*4] = _temp;
	_temp = _mtemp[0+2+2*4];
	_omat[0+2+2*4] = _temp;
	_temp = _mtemp[0+3+3*4];
	_omat[0+3+3*4] = _temp;
}
...
void GetFixefErrToThetaMap(int* _iaTheta, int* _nth){
	int _n = 0;
	_iaTheta[0] = _n++; // GAMMA
	_iaTheta[1] = _n++; // CMultStdev
	_iaTheta[2] = _n++; // tvVmax
	_iaTheta[3] = _n++; // tvKm
	_iaTheta[4] = _n++; // tvV
	_iaTheta[5] = _n++; // tvKa
	_iaTheta[6] = _n++; // K12
...
void   TypXTheta(double* _fixef, double* _thvec, int* _nth){
	...
	_thvec[_n++] = fabs(_fixef[9]);
	// unfrozen error sigmas 
	_thvec[_n++] = fabs(_fixef[10]);
	*_nth = _n;
}
...
void   GetPackedIndexRanCov(int* _iRan, int* _jRan, int* _index){
	...
	*_index = -1;
	...
	{
		if (*_iRan==3 && *_jRan==3){*_index = _n; return;}
		_n++;
	}
}
...
static double _tempResult[4];
static double _tempArg[4];
int _bTempValid = 0;
void InitTemp(){ int _i;
	for (_i = 0; _i < 4; _i++) _tempResult[_i] = -999999999.0;
...
void EvalSParms(double* zzY){
	...
	Ka = (tvKa * _exp_cached(nKa, &_tempArg[3], &_tempResult[3]));
...
void Deriv(long* _pN, double* _pt, double zzY[], double zzR[], double zzIRate[]){
	...
	zzR_Aa = ( -(Ka) * Aa);
	zzR_Aa += _Aa_irate;
	zzR_A1 = ((((((Ka * Aa) + (K21 * A2)) - (K12 * A1)) + (K31 * A3)) - (K12 * A2)) - ((Vmax * pow(C, GAMMA)) / (pow(Km, GAMMA) + pow(C, GAMMA))));
	zzR_A1 += _A1_irate;
	zzR_A2 = ((K12 * A1) - (K21 * A2));
	zzR_A2 += _A2_irate;
	zzR_A3 = ((K13 * A1) - (K31 * A3));
	zzR_A3 += _A3_irate;
	zzR[4] = 0; // extra null equation
}
	//  -((tvKa * exp(nKa)))
	// (tvKa * exp(nKa))
	// 0.0
	// 0.0
	// 0.0
	// ( -(K12) - ((((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((tvVmax * exp(nVmax)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) - (((tvVmax * exp(nVmax)) * pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) / ((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)))))  is not constant
	// K12
	// K13
	// 0.0
	// (K21 - K12)
...
void Jacobian(int* pN, double* pt, double zzY[], double zzIRate[], int* pML, int* pMU, double* _jacobian, int* pNRJ){
	...
	// rate terms for Aa
		// w.r.t. Aa
	_jacobian[0 + _nrj*0] =  -((tvKa * __temp[0]));
		// forcing rate for Aa
	_jacobian[0 + _nrj*4] = 0.0 + zzIRate[0];
	// rate terms for A1
		// w.r.t. Aa
	_jacobian[1 + _nrj*0] = (tvKa * __temp[0]);
		// w.r.t. A1
	_jacobian[1 + _nrj*1] = ( -(K12) - ((((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((tvVmax * __temp[3]) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))) - (((tvVmax * __temp[3]) * pow((A1 / (tvV * __temp[2])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))) / ((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * (pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)))));
		// w.r.t. A2
	_jacobian[1 + _nrj*2] = (K21 - K12);
		// w.r.t. A3
	_jacobian[1 + _nrj*3] = K31;
		// forcing rate for A1
	_jacobian[1 + _nrj*4] = 0.0 + zzIRate[1];
	// rate terms for A2
		// w.r.t. A1
	...
	_jacobian[2 + _nrj*2] =  -(K21);
		// forcing rate for A2
	_jacobian[2 + _nrj*4] = 0.0 + zzIRate[2];
	// rate terms for A3
		// w.r.t. A1
	_jacobian[3 + _nrj*1] = K13;
		// w.r.t. A3
	_jacobian[3 + _nrj*3] =  -(K31);
		// forcing rate for A3
	_jacobian[3 + _nrj*4] = 0.0 + zzIRate[3];
}
void ResetTime(){
	curTime = 0;
...
int GetCompartmentNumber(const char* _nm){
	...
	else if (_nm[0]=='A' && strcmp(_nm, "A3")==0) _i = 3;
	return _i;
}
...
int GetVarValue(const char* _nm, double* _pv, double zzY[]){
    ...
    if (_nm[0]=='A' && strcmp(_nm, "A3")==0) {
        *_pv = A3;
        return 1;
    }
    if (_nm[0]=='G' && strcmp(_nm, "GAMMA")==0) {
        *_pv = GAMMA;
        return 1;
    }
    ...
    if (_nm[0]=='K' && strcmp(_nm, "K13")==0) {
        *_pv = K13;
        return 1;
    }
    if (_nm[0]=='K' && strcmp(_nm, "K31")==0) {
        *_pv = K31;
        return 1;
    }
...
{
	static const int is_saem[] = {1, 1, 1, 1};
    return is_saem[* idx];
}
...
void GetSaemCMatrixSize(int* nBaseEta, int* nExtraEta, int* nTheta){
	*nBaseEta = *nTheta = *nExtraEta = 0;
	...
	if ((-1 == -1)||_ISENABLED(-1)){(*nExtraEta)++; (*nTheta)++;} // bare theta GAMMA
	...
	if ((-1 == -1)||_ISENABLED(-1)){(*nExtraEta)++; (*nTheta)++;} // bare theta K13

	if ((-1 == -1)||_ISENABLED(-1)){(*nExtraEta)++; (*nTheta)++;} // bare theta K31
...
void GetSaemCMatrix(double* daCMat, int* theta2eta){
	...
	// for structural parameter log_Ka
	daCMat[_iEta + _jTheta * _nEta] = 1; // eta nKa, theta log_tvKa
	theta2eta[_jTheta] = _iEta;
	_jTheta++;
	_iEta++;
	...
	{
		daCMat[_iEta + _jTheta * _nEta] = 1; // extra eta for bare theta GAMMA
		theta2eta[_jTheta] = -1;
		_iEta++;
		_jTheta++;
	}
	...
	{
		daCMat[_iEta + _jTheta * _nEta] = 1; // extra eta for bare theta K13
		theta2eta[_jTheta] = -1;
		_iEta++;
		_jTheta++;
	}
	...
	{
		daCMat[_iEta + _jTheta * _nEta] = 1; // extra eta for bare theta K31
		theta2eta[_jTheta] = -1;
		_iEta++;
		_jTheta++;
	}
...
void GetSaemThetaInfo(int* iaCColToITheta, int* baCColIsLog){
	int _jCTheta = 0;
	int _nTheta = 0;
	int _iaIFixefErrToITheta[10+1];
	int _i;
	for (_i = 0; _i < 10+1; _i++) _iaIFixefErrToITheta[_i] = -1;
	GetFixefErrToThetaMap(_iaIFixefErrToITheta, &_nTheta);

	iaCColToITheta[_jCTheta] = _iaIFixefErrToITheta[2]; // tvVmax
	baCColIsLog[_jCTheta] = 1;
	_jCTheta++;

	iaCColToITheta[_jCTheta] = _iaIFixefErrToITheta[4]; // tvV
	baCColIsLog[_jCTheta] = 1;
	_jCTheta++;

	iaCColToITheta[_jCTheta] = _iaIFixefErrToITheta[3]; // tvKm
	baCColIsLog[_jCTheta] = 1;
	_jCTheta++;

	iaCColToITheta[_jCTheta] = _iaIFixefErrToITheta[5]; // tvKa
	baCColIsLog[_jCTheta] = 1;
	_jCTheta++;

	// for additional bare thetas
	{
		iaCColToITheta[_jCTheta] = _iaIFixefErrToITheta[0]; // GAMMA
		baCColIsLog[_jCTheta] = 0;
		_jCTheta++;
	}

	{
		iaCColToITheta[_jCTheta] = _iaIFixefErrToITheta[1]; // CMultStdev
		baCColIsLog[_jCTheta] = 0;
		_jCTheta++;
	}

	{
		iaCColToITheta[_jCTheta] = _iaIFixefErrToITheta[6]; // K12
		baCColIsLog[_jCTheta] = 0;
		_jCTheta++;
	}

	{
		iaCColToITheta[_jCTheta] = _iaIFixefErrToITheta[7]; // K21
		baCColIsLog[_jCTheta] = 0;
		_jCTheta++;
	}

	{
		iaCColToITheta[_jCTheta] = _iaIFixefErrToITheta[8]; // K13
		baCColIsLog[_jCTheta] = 0;
		_jCTheta++;
	}

	{
		iaCColToITheta[_jCTheta] = _iaIFixefErrToITheta[9]; // K31
		baCColIsLog[_jCTheta] = 0;
		_jCTheta++;
	}
...
#define     _A3_nVmax       zzY1[2]
#define zzR__A3_nVmax       zzR1[2]

#define     _A1_nKm       zzY1[3]
#define zzR__A1_nKm       zzR1[3]

#define     _A2_nKm       zzY1[4]
#define zzR__A2_nKm       zzR1[4]

#define     _A3_nKm       zzY1[5]
#define zzR__A3_nKm       zzR1[5]

#define     _A1_nV       zzY1[6]
#define zzR__A1_nV       zzR1[6]

#define     _A2_nV       zzY1[7]
#define zzR__A2_nV       zzR1[7]

#define     _A3_nV       zzY1[8]
#define zzR__A3_nV       zzR1[8]

#define     _Aa_nKa       zzY1[9]
#define zzR__Aa_nKa       zzR1[9]

#define     _A1_nKa       zzY1[10]
#define zzR__A1_nKa       zzR1[10]

#define     _A2_nKa       zzY1[11]
#define zzR__A2_nKa       zzR1[11]

#define     _A3_nKa       zzY1[12]
#define zzR__A3_nKa       zzR1[12]

#define NINTEG_GRADIENT_DERIV 13
...
void GradientDeriv(long* _pN, double* _pt, double zzY[], double zzR[], double zzIRate[])
{
	...
	zzR_A3 += _A3_irate;
	zzR[4] = 0; // extra null equation
	zzR__A1_nVmax = (((((Ka * _Aa_nVmax) + (( -(K12) - ((((pow(Km, GAMMA) + pow((A1 / V), GAMMA)) * (Vmax * ((GAMMA * pow((A1 / V), (GAMMA - 1.0))) / V))) - ((Vmax * pow((A1 / V), GAMMA)) * ((GAMMA * pow((A1 / V), (GAMMA - 1.0))) / V))) / ((pow(Km, GAMMA) + pow((A1 / V), GAMMA)) * (pow(Km, GAMMA) + pow((A1 / V), GAMMA))))) * _A1_nVmax)) + ((K21 - K12) * _A2_nVmax)) + (K31 * _A3_nVmax)) +  -((((pow(Km, GAMMA) + pow((A1 / V), GAMMA)) * (pow((A1 / V), GAMMA) * Vmax)) / ((pow(Km, GAMMA) + pow((A1 / V), GAMMA)) * (pow(Km, GAMMA) + pow((A1 / V), GAMMA))))));
	zzR__A2_nVmax = ((K12 * _A1_nVmax) + ( -(K21) * _A2_nVmax));
	zzR__A3_nVmax = ((K13 * _A1_nVmax) + ( -(K31) * _A3_nVmax));
	zzR__A1_nKm = (((((Ka * _Aa_nKm) + (( -(K12) - ((((pow(Km, GAMMA) + pow((A1 / V), GAMMA)) * (Vmax * ((GAMMA * pow((A1 / V), (GAMMA - 1.0))) / V))) - ((Vmax * pow((A1 / V), GAMMA)) * ((GAMMA * pow((A1 / V), (GAMMA - 1.0))) / V))) / ((pow(Km, GAMMA) + pow((A1 / V), GAMMA)) * (pow(Km, GAMMA) + pow((A1 / V), GAMMA))))) * _A1_nKm)) + ((K21 - K12) * _A2_nKm)) + (K31 * _A3_nKm)) +  -(((((pow(Km, GAMMA) + pow((A1 / V), GAMMA)) * (Vmax * ((GAMMA * pow((A1 / V), (GAMMA - 1.0))) * ( -((A1 * V)) / (V * V))))) - ((Vmax * pow((A1 / V), GAMMA)) * ((GAMMA * pow((A1 / V), (GAMMA - 1.0))) * ( -((A1 * V)) / (V * V))))) / ((pow(Km, GAMMA) + pow((A1 / V), GAMMA)) * (pow(Km, GAMMA) + pow((A1 / V), GAMMA))))));
	zzR__A2_nKm = ((K12 * _A1_nKm) + ( -(K21) * _A2_nKm));
	zzR__A3_nKm = ((K13 * _A1_nKm) + ( -(K31) * _A3_nKm));
	zzR__A1_nV = (((((Ka * _Aa_nV) + (( -(K12) - ((((pow(Km, GAMMA) + pow((A1 / V), GAMMA)) * (Vmax * ((GAMMA * pow((A1 / V), (GAMMA - 1.0))) / V))) - ((Vmax * pow((A1 / V), GAMMA)) * ((GAMMA * pow((A1 / V), (GAMMA - 1.0))) / V))) / ((pow(Km, GAMMA) + pow((A1 / V), GAMMA)) * (pow(Km, GAMMA) + pow((A1 / V), GAMMA))))) * _A1_nV)) + ((K21 - K12) * _A2_nV)) + (K31 * _A3_nV)) +  -(( -(((Vmax * pow((A1 / V), GAMMA)) * ((GAMMA * pow(Km, (GAMMA - 1.0))) * Km))) / ((pow(Km, GAMMA) + pow((A1 / V), GAMMA)) * (pow(Km, GAMMA) + pow((A1 / V), GAMMA))))));
	zzR__A2_nV = ((K12 * _A1_nV) + ( -(K21) * _A2_nV));
	zzR__A3_nV = ((K13 * _A1_nV) + ( -(K31) * _A3_nV));
	zzR__Aa_nKa = (( -(Ka) * _Aa_nKa) + (Aa *  -(Ka)));
	zzR__A1_nKa = (((((Ka * _Aa_nKa) + (( -(K12) - ((((pow(Km, GAMMA) + pow((A1 / V), GAMMA)) * (Vmax * ((GAMMA * pow((A1 / V), (GAMMA - 1.0))) / V))) - ((Vmax * pow((A1 / V), GAMMA)) * ((GAMMA * pow((A1 / V), (GAMMA - 1.0))) / V))) / ((pow(Km, GAMMA) + pow((A1 / V), GAMMA)) * (pow(Km, GAMMA) + pow((A1 / V), GAMMA))))) * _A1_nKa)) + ((K21 - K12) * _A2_nKa)) + (K31 * _A3_nKa)) + (Aa * Ka));
	zzR__A2_nKa = ((K12 * _A1_nKa) + ( -(K21) * _A2_nKa));
	zzR__A3_nKa = ((K13 * _A1_nKa) + ( -(K31) * _A3_nKa));
}
	//[0,0]  -((tvKa * exp(nKa)))
	//[1,0] (tvKa * exp(nKa))
	//[2,0] 0.0
	...
	//[0,1] 0.0
	//[1,1] ( -(K12) - ((((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((tvVmax * exp(nVmax)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) - (((tvVmax * exp(nVmax)) * pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) / ((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)))))  is not constant
	//[2,1] K12
	//[3,1] K13
	//[5,1] ((_A1_nVmax *  -((((((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA))) * (((((tvVmax * exp(nVmax)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm)))) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm)))) + ((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((tvVmax * exp(nVmax)) * (((tvV * exp(nKm)) * (GAMMA * (((GAMMA - 1.0) * pow((A1 / (tvV * exp(nKm))), ((GAMMA - 1.0) - 1.0))) / (tvV * exp(nKm))))) / ((tvV * exp(nKm)) * (tvV * exp(nKm))))))) - ((((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))) * ((tvVmax * exp(nVmax)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) + (((tvVmax * exp(nVmax)) * pow((A1 / (tvV * exp(nKm))), GAMMA)) * (((tvV * exp(nKm)) * (GAMMA * (((GAMMA - 1.0) * pow((A1 / (tvV * exp(nKm))), ((GAMMA - 1.0) - 1.0))) / (tvV * exp(nKm))))) / ((tvV * exp(nKm)) * (tvV * exp(nKm)))))))) - ((((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((tvVmax * exp(nVmax)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) - (((tvVmax * exp(nVmax)) * pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) * (((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm)))) + ((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))))) / (((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA))) * ((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA))))))) +  -((((((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA))) * (((pow((A1 / (tvV * exp(nKm))), GAMMA) * (tvVmax * exp(nVmax))) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm)))) + ((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((tvVmax * exp(nVmax)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))))) - (((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((A1 / (tvV * exp(nKm))), GAMMA) * (tvVmax * exp(nVmax)))) * (((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm)))) + ((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))))) / (((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA))) * ((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)))))))  is not constant
	//[6,1] 0.0
	//[7,1] 0.0
	//[8,1] ((_A1_nKm *  -((((((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA))) * (((((tvVmax * exp(nVmax)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm)))) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm)))) + ((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((tvVmax * exp(nVmax)) * (((tvV * exp(nKm)) * (GAMMA * (((GAMMA - 1.0) * pow((A1 / (tvV * exp(nKm))), ((GAMMA - 1.0) - 1.0))) / (tvV * exp(nKm))))) / ((tvV * exp(nKm)) * (tvV * exp(nKm))))))) - ((((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))) * ((tvVmax * exp(nVmax)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) + (((tvVmax * exp(nVmax)) * pow((A1 / (tvV * exp(nKm))), GAMMA)) * (((tvV * exp(nKm)) * (GAMMA * (((GAMMA - 1.0) * pow((A1 / (tvV * exp(nKm))), ((GAMMA - 1.0) - 1.0))) / (tvV * exp(nKm))))) / ((tvV * exp(nKm)) * (tvV * exp(nKm)))))))) - ((((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((tvVmax * exp(nVmax)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) - (((tvVmax * exp(nVmax)) * pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) * (((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm)))) + ((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))))) / (((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA))) * ((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA))))))) +  -((((((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA))) * (((((tvVmax * exp(nVmax)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) * ( -((A1 * (tvV * exp(nKm)))) / ((tvV * exp(nKm)) * (tvV * exp(nKm)))))) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm)))) + ((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((tvVmax * exp(nVmax)) * ((( -((A1 * (tvV * exp(nKm)))) / ((tvV * exp(nKm)) * (tvV * exp(nKm)))) * (GAMMA * (((GAMMA - 1.0) * pow((A1 / (tvV * exp(nKm))), ((GAMMA - 1.0) - 1.0))) / (tvV * exp(nKm))))) + ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) * ((((tvV * exp(nKm)) * (tvV * exp(nKm))) *  -((tvV * exp(nKm)))) / (((tvV * exp(nKm)) * (tvV * exp(nKm))) * ((tvV * exp(nKm)) * (tvV * exp(nKm)))))))))) - ((((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) * ( -((A1 * (tvV * exp(nKm)))) / ((tvV * exp(nKm)) * (tvV * exp(nKm))))) * ((tvVmax * exp(nVmax)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) + (((tvVmax * exp(nVmax)) * pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((( -((A1 * (tvV * exp(nKm)))) / ((tvV * exp(nKm)) * (tvV * exp(nKm)))) * (GAMMA * (((GAMMA - 1.0) * pow((A1 / (tvV * exp(nKm))), ((GAMMA - 1.0) - 1.0))) / (tvV * exp(nKm))))) + ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) * ((((tvV * exp(nKm)) * (tvV * exp(nKm))) *  -((tvV * exp(nKm)))) / (((tvV * exp(nKm)) * (tvV * exp(nKm))) * ((tvV * exp(nKm)) * (tvV * exp(nKm))))))))))) - ((((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((tvVmax * exp(nVmax)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) * ( -((A1 * (tvV * exp(nKm)))) / ((tvV * exp(nKm)) * (tvV * exp(nKm))))))) - (((tvVmax * exp(nVmax)) * pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) * ( -((A1 * (tvV * exp(nKm)))) / ((tvV * exp(nKm)) * (tvV * exp(nKm))))))) * (((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm)))) + ((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))))) / (((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA))) * ((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)))))))  is not constant
	//[9,1] 0.0
	//[10,1] 0.0
	//[11,1] ((_A1_nV *  -((((((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA))) * (((((tvVmax * exp(nVmax)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm)))) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm)))) + ((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((tvVmax * exp(nVmax)) * (((tvV * exp(nKm)) * (GAMMA * (((GAMMA - 1.0) * pow((A1 / (tvV * exp(nKm))), ((GAMMA - 1.0) - 1.0))) / (tvV * exp(nKm))))) / ((tvV * exp(nKm)) * (tvV * exp(nKm))))))) - ((((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))) * ((tvVmax * exp(nVmax)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) + (((tvVmax * exp(nVmax)) * pow((A1 / (tvV * exp(nKm))), GAMMA)) * (((tvV * exp(nKm)) * (GAMMA * (((GAMMA - 1.0) * pow((A1 / (tvV * exp(nKm))), ((GAMMA - 1.0) - 1.0))) / (tvV * exp(nKm))))) / ((tvV * exp(nKm)) * (tvV * exp(nKm)))))))) - ((((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((tvVmax * exp(nVmax)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) - (((tvVmax * exp(nVmax)) * pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) * (((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm)))) + ((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))))) / (((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA))) * ((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA))))))) +  -((((((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA))) *  -((((GAMMA * pow((tvKm * exp(nV)), (GAMMA - 1.0))) * (tvKm * exp(nV))) * ((tvVmax * exp(nVmax)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))))) - ( -((((tvVmax * exp(nVmax)) * pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((GAMMA * pow((tvKm * exp(nV)), (GAMMA - 1.0))) * (tvKm * exp(nV))))) * (((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm)))) + ((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))))) / (((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA))) * ((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)))))))  is not constant
	//[12,1] 0.0
	//[13,1] 0.0
	//[14,1] 0.0
	//[15,1] (_A1_nKa *  -((((((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA))) * (((((tvVmax * exp(nVmax)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm)))) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm)))) + ((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((tvVmax * exp(nVmax)) * (((tvV * exp(nKm)) * (GAMMA * (((GAMMA - 1.0) * pow((A1 / (tvV * exp(nKm))), ((GAMMA - 1.0) - 1.0))) / (tvV * exp(nKm))))) / ((tvV * exp(nKm)) * (tvV * exp(nKm))))))) - ((((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))) * ((tvVmax * exp(nVmax)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) + (((tvVmax * exp(nVmax)) * pow((A1 / (tvV * exp(nKm))), GAMMA)) * (((tvV * exp(nKm)) * (GAMMA * (((GAMMA - 1.0) * pow((A1 / (tvV * exp(nKm))), ((GAMMA - 1.0) - 1.0))) / (tvV * exp(nKm))))) / ((tvV * exp(nKm)) * (tvV * exp(nKm)))))))) - ((((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((tvVmax * exp(nVmax)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) - (((tvVmax * exp(nVmax)) * pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) * (((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm)))) + ((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))))) / (((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA))) * ((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)))))))  is not constant
	//[16,1] 0.0
	//[17,1] 0.0
	...
	//[2,5] 0.0
	//[3,5] 0.0
	//[5,5] ( -(K12) - ((((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((tvVmax * exp(nVmax)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) - (((tvVmax * exp(nVmax)) * pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) / ((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)))))  is not constant
	//[6,5] K12
	//[7,5] K13
	//[8,5] 0.0
	//[9,5] 0.0
	...
	//[6,8] 0.0
	//[7,8] 0.0
	//[8,8] ( -(K12) - ((((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((tvVmax * exp(nVmax)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) - (((tvVmax * exp(nVmax)) * pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) / ((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)))))  is not constant
	//[9,8] K12
	//[10,8] K13
	//[11,8] 0.0
	//[12,8] 0.0
	...
	//[9,11] 0.0
	//[10,11] 0.0
	//[11,11] ( -(K12) - ((((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((tvVmax * exp(nVmax)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) - (((tvVmax * exp(nVmax)) * pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) / ((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)))))  is not constant
	//[12,11] K12
	//[13,11] K13
	//[14,11] 0.0
	//[15,11] 0.0
	...
	//[13,15] 0.0
	//[14,15] 0.0
	//[15,15] ( -(K12) - ((((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((tvVmax * exp(nVmax)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) - (((tvVmax * exp(nVmax)) * pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) / ((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)))))  is not constant
	//[16,15] K12
	//[17,15] K13
...
void GradientJacobian(int* pN, double* pt, double zzY[], double zzIRate[], int* pML, int* pMU, double* _gjacobian, int* pNRJ){
	...
	// rate terms for Aa
		// w.r.t. Aa
	_gjacobian[0 + _nrj*0] =  -((tvKa * __temp[0]));
		// forcing rate for Aa
	_gjacobian[0 + _nrj*4] = 0.0 + zzIRate[0];
	// rate terms for A1
		// w.r.t. Aa
	_gjacobian[1 + _nrj*0] = (tvKa * __temp[0]);
		// w.r.t. A1
	_gjacobian[1 + _nrj*1] = ( -(K12) - ((((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((tvVmax * __temp[3]) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))) - (((tvVmax * __temp[3]) * pow((A1 / (tvV * __temp[2])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))) / ((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * (pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)))));
		// w.r.t. A2
	_gjacobian[1 + _nrj*2] = (K21 - K12);
		// w.r.t. A3
	_gjacobian[1 + _nrj*3] = K31;
		// forcing rate for A1
	_gjacobian[1 + _nrj*4] = 0.0 + zzIRate[1];
	// rate terms for A2
		// w.r.t. A1
	...
	_gjacobian[2 + _nrj*2] =  -(K21);
		// forcing rate for A2
	_gjacobian[2 + _nrj*4] = 0.0 + zzIRate[2];
	// rate terms for A3
		// w.r.t. A1
	_gjacobian[3 + _nrj*1] = K13;
		// w.r.t. A3
	_gjacobian[3 + _nrj*3] =  -(K31);
		// forcing rate for A3
	_gjacobian[3 + _nrj*4] = 0.0 + zzIRate[3];
	// rate terms for zzR[4]
	// rate terms for _A1_nVmax
		// w.r.t. A1
	_gjacobian[5 + _nrj*1] = ((_A1_nVmax *  -((((((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * (pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA))) * (((((tvVmax * __temp[3]) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2]))) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2]))) + ((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((tvVmax * __temp[3]) * (((tvV * __temp[2]) * (GAMMA * (((GAMMA - 1.0) * pow((A1 / (tvV * __temp[2])), ((GAMMA - 1.0) - 1.0))) / (tvV * __temp[2])))) / ((tvV * __temp[2]) * (tvV * __temp[2])))))) - ((((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])) * ((tvVmax * __temp[3]) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))) + (((tvVmax * __temp[3]) * pow((A1 / (tvV * __temp[2])), GAMMA)) * (((tvV * __temp[2]) * (GAMMA * (((GAMMA - 1.0) * pow((A1 / (tvV * __temp[2])), ((GAMMA - 1.0) - 1.0))) / (tvV * __temp[2])))) / ((tvV * __temp[2]) * (tvV * __temp[2]))))))) - ((((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((tvVmax * __temp[3]) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))) - (((tvVmax * __temp[3]) * pow((A1 / (tvV * __temp[2])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))) * (((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2]))) + ((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))))) / (((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * (pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA))) * ((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * (pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA))))))) +  -((((((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * (pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA))) * (((pow((A1 / (tvV * __temp[2])), GAMMA) * (tvVmax * __temp[3])) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2]))) + ((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((tvVmax * __temp[3]) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))))) - (((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * (pow((A1 / (tvV * __temp[2])), GAMMA) * (tvVmax * __temp[3]))) * (((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2]))) + ((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))))) / (((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * (pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA))) * ((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * (pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)))))));
		// w.r.t. _A1_nVmax
	_gjacobian[5 + _nrj*5] = ( -(K12) - ((((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((tvVmax * __temp[3]) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))) - (((tvVmax * __temp[3]) * pow((A1 / (tvV * __temp[2])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))) / ((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * (pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)))));
		// w.r.t. _A2_nVmax
	_gjacobian[5 + _nrj*6] = (K21 - K12);
		// w.r.t. _A3_nVmax
	_gjacobian[5 + _nrj*7] = K31;
	// rate terms for _A2_nVmax
		// w.r.t. _A1_nVmax
	_gjacobian[6 + _nrj*5] = K12;
		// w.r.t. _A2_nVmax
	_gjacobian[6 + _nrj*6] =  -(K21);
	// rate terms for _A3_nVmax
		// w.r.t. _A1_nVmax
	_gjacobian[7 + _nrj*5] = K13;
		// w.r.t. _A3_nVmax
	_gjacobian[7 + _nrj*7] =  -(K31);
	// rate terms for _A1_nKm
		// w.r.t. A1
	_gjacobian[8 + _nrj*1] = ((_A1_nKm *  -((((((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * (pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA))) * (((((tvVmax * __temp[3]) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2]))) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2]))) + ((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((tvVmax * __temp[3]) * (((tvV * __temp[2]) * (GAMMA * (((GAMMA - 1.0) * pow((A1 / (tvV * __temp[2])), ((GAMMA - 1.0) - 1.0))) / (tvV * __temp[2])))) / ((tvV * __temp[2]) * (tvV * __temp[2])))))) - ((((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])) * ((tvVmax * __temp[3]) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))) + (((tvVmax * __temp[3]) * pow((A1 / (tvV * __temp[2])), GAMMA)) * (((tvV * __temp[2]) * (GAMMA * (((GAMMA - 1.0) * pow((A1 / (tvV * __temp[2])), ((GAMMA - 1.0) - 1.0))) / (tvV * __temp[2])))) / ((tvV * __temp[2]) * (tvV * __temp[2]))))))) - ((((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((tvVmax * __temp[3]) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))) - (((tvVmax * __temp[3]) * pow((A1 / (tvV * __temp[2])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))) * (((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2]))) + ((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))))) / (((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * (pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA))) * ((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * (pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA))))))) +  -((((((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * (pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA))) * (((((tvVmax * __temp[3]) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) * ( -((A1 * (tvV * __temp[2]))) / ((tvV * __temp[2]) * (tvV * __temp[2]))))) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2]))) + ((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((tvVmax * __temp[3]) * ((( -((A1 * (tvV * __temp[2]))) / ((tvV * __temp[2]) * (tvV * __temp[2]))) * (GAMMA * (((GAMMA - 1.0) * pow((A1 / (tvV * __temp[2])), ((GAMMA - 1.0) - 1.0))) / (tvV * __temp[2])))) + ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) * ((((tvV * __temp[2]) * (tvV * __temp[2])) *  -((tvV * __temp[2]))) / (((tvV * __temp[2]) * (tvV * __temp[2])) * ((tvV * __temp[2]) * (tvV * __temp[2]))))))))) - ((((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) * ( -((A1 * (tvV * __temp[2]))) / ((tvV * __temp[2]) * (tvV * __temp[2])))) * ((tvVmax * __temp[3]) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))) + (((tvVmax * __temp[3]) * pow((A1 / (tvV * __temp[2])), GAMMA)) * ((( -((A1 * (tvV * __temp[2]))) / ((tvV * __temp[2]) * (tvV * __temp[2]))) * (GAMMA * (((GAMMA - 1.0) * pow((A1 / (tvV * __temp[2])), ((GAMMA - 1.0) - 1.0))) / (tvV * __temp[2])))) + ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) * ((((tvV * __temp[2]) * (tvV * __temp[2])) *  -((tvV * __temp[2]))) / (((tvV * __temp[2]) * (tvV * __temp[2])) * ((tvV * __temp[2]) * (tvV * __temp[2])))))))))) - ((((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((tvVmax * __temp[3]) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) * ( -((A1 * (tvV * __temp[2]))) / ((tvV * __temp[2]) * (tvV * __temp[2])))))) - (((tvVmax * __temp[3]) * pow((A1 / (tvV * __temp[2])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) * ( -((A1 * (tvV * __temp[2]))) / ((tvV * __temp[2]) * (tvV * __temp[2])))))) * (((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2]))) + ((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))))) / (((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * (pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA))) * ((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * (pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)))))));
		// w.r.t. _A1_nKm
	_gjacobian[8 + _nrj*8] = ( -(K12) - ((((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((tvVmax * __temp[3]) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))) - (((tvVmax * __temp[3]) * pow((A1 / (tvV * __temp[2])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))) / ((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * (pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)))));
		// w.r.t. _A2_nKm
	_gjacobian[8 + _nrj*9] = (K21 - K12);
		// w.r.t. _A3_nKm
	_gjacobian[8 + _nrj*10] = K31;
	// rate terms for _A2_nKm
		// w.r.t. _A1_nKm
	_gjacobian[9 + _nrj*8] = K12;
		// w.r.t. _A2_nKm
	_gjacobian[9 + _nrj*9] =  -(K21);
	// rate terms for _A3_nKm
		// w.r.t. _A1_nKm
	_gjacobian[10 + _nrj*8] = K13;
		// w.r.t. _A3_nKm
	_gjacobian[10 + _nrj*10] =  -(K31);
	// rate terms for _A1_nV
		// w.r.t. A1
	_gjacobian[11 + _nrj*1] = ((_A1_nV *  -((((((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * (pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA))) * (((((tvVmax * __temp[3]) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2]))) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2]))) + ((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((tvVmax * __temp[3]) * (((tvV * __temp[2]) * (GAMMA * (((GAMMA - 1.0) * pow((A1 / (tvV * __temp[2])), ((GAMMA - 1.0) - 1.0))) / (tvV * __temp[2])))) / ((tvV * __temp[2]) * (tvV * __temp[2])))))) - ((((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])) * ((tvVmax * __temp[3]) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))) + (((tvVmax * __temp[3]) * pow((A1 / (tvV * __temp[2])), GAMMA)) * (((tvV * __temp[2]) * (GAMMA * (((GAMMA - 1.0) * pow((A1 / (tvV * __temp[2])), ((GAMMA - 1.0) - 1.0))) / (tvV * __temp[2])))) / ((tvV * __temp[2]) * (tvV * __temp[2]))))))) - ((((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((tvVmax * __temp[3]) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))) - (((tvVmax * __temp[3]) * pow((A1 / (tvV * __temp[2])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))) * (((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2]))) + ((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))))) / (((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * (pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA))) * ((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * (pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA))))))) +  -((((((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * (pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA))) *  -((((GAMMA * pow((tvKm * __temp[1]), (GAMMA - 1.0))) * (tvKm * __temp[1])) * ((tvVmax * __temp[3]) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))))) - ( -((((tvVmax * __temp[3]) * pow((A1 / (tvV * __temp[2])), GAMMA)) * ((GAMMA * pow((tvKm * __temp[1]), (GAMMA - 1.0))) * (tvKm * __temp[1])))) * (((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2]))) + ((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))))) / (((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * (pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA))) * ((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * (pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)))))));
		// w.r.t. _A1_nV
	_gjacobian[11 + _nrj*11] = ( -(K12) - ((((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((tvVmax * __temp[3]) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))) - (((tvVmax * __temp[3]) * pow((A1 / (tvV * __temp[2])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))) / ((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * (pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)))));
		// w.r.t. _A2_nV
	_gjacobian[11 + _nrj*12] = (K21 - K12);
		// w.r.t. _A3_nV
	_gjacobian[11 + _nrj*13] = K31;
	// rate terms for _A2_nV
		// w.r.t. _A1_nV
	_gjacobian[12 + _nrj*11] = K12;
		// w.r.t. _A2_nV
	_gjacobian[12 + _nrj*12] =  -(K21);
	// rate terms for _A3_nV
		// w.r.t. _A1_nV
	_gjacobian[13 + _nrj*11] = K13;
		// w.r.t. _A3_nV
	_gjacobian[13 + _nrj*13] =  -(K31);
	// rate terms for _Aa_nKa
		// w.r.t. Aa
	_gjacobian[14 + _nrj*0] =  -((tvKa * __temp[0]));
		// w.r.t. _Aa_nKa
	_gjacobian[14 + _nrj*14] =  -((tvKa * __temp[0]));
	// rate terms for _A1_nKa
		// w.r.t. Aa
	_gjacobian[15 + _nrj*0] = (tvKa * __temp[0]);
		// w.r.t. A1
	_gjacobian[15 + _nrj*1] = (_A1_nKa *  -((((((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * (pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA))) * (((((tvVmax * __temp[3]) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2]))) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2]))) + ((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((tvVmax * __temp[3]) * (((tvV * __temp[2]) * (GAMMA * (((GAMMA - 1.0) * pow((A1 / (tvV * __temp[2])), ((GAMMA - 1.0) - 1.0))) / (tvV * __temp[2])))) / ((tvV * __temp[2]) * (tvV * __temp[2])))))) - ((((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])) * ((tvVmax * __temp[3]) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))) + (((tvVmax * __temp[3]) * pow((A1 / (tvV * __temp[2])), GAMMA)) * (((tvV * __temp[2]) * (GAMMA * (((GAMMA - 1.0) * pow((A1 / (tvV * __temp[2])), ((GAMMA - 1.0) - 1.0))) / (tvV * __temp[2])))) / ((tvV * __temp[2]) * (tvV * __temp[2]))))))) - ((((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((tvVmax * __temp[3]) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))) - (((tvVmax * __temp[3]) * pow((A1 / (tvV * __temp[2])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))) * (((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2]))) + ((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))))) / (((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * (pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA))) * ((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * (pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)))))));
		// w.r.t. _Aa_nKa
	_gjacobian[15 + _nrj*14] = (tvKa * __temp[0]);
		// w.r.t. _A1_nKa
	_gjacobian[15 + _nrj*15] = ( -(K12) - ((((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * ((tvVmax * __temp[3]) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))) - (((tvVmax * __temp[3]) * pow((A1 / (tvV * __temp[2])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[2])), (GAMMA - 1.0))) / (tvV * __temp[2])))) / ((pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)) * (pow((tvKm * __temp[1]), GAMMA) + pow((A1 / (tvV * __temp[2])), GAMMA)))));
		// w.r.t. _A2_nKa
	_gjacobian[15 + _nrj*16] = (K21 - K12);
		// w.r.t. _A3_nKa
	_gjacobian[15 + _nrj*17] = K31;
	// rate terms for _A2_nKa
		// w.r.t. _A1_nKa
	_gjacobian[16 + _nrj*15] = K12;
		// w.r.t. _A2_nKa
	_gjacobian[16 + _nrj*16] =  -(K21);
	// rate terms for _A3_nKa
		// w.r.t. _A1_nKa
	_gjacobian[17 + _nrj*15] = K13;
		// w.r.t. _A3_nKa
	_gjacobian[17 + _nrj*17] =  -(K31);
}
...
void NewGetLL(int bGenGradients, double _dLL[], double _dLLdEta[], double zzY[], double zzIRate[], int* pnObs, double _t, const char* _nm, double _v1, int bBql)
{
    ...
    if (_nm[0]=='C' && strcmp(_nm, "CObs")==0){
		...
		if (bGenGradients){
			...
			if (ALLOWLOGTRANSFORM && bEnableLogTransform){
				_dYdEta = ((V * _A1_nKa) / (V * V)) / _ypred; // Get dY / dnKa
				_dSdEta = 0; // Get dS / dnKa
			} else {
				_dYdEta = ((V * _A1_nKa) / (V * V)); // Get dY / dnKa
				_dSdEta = 0.0; // Get dS / dnKa
			}
			if (ALLOWLOGTRANSFORM && bEnableLogTransform){
				if (bBql){
					_dLL2 = lphi(logfl(_v1) - logfl(_ypred + _dYdEta*_del), (1+_dSdEta*_del)*_std);
					_dLLdEta1[3] = (_dLL2 - _dLL1)/_del;
				} else {
					_dLLdEta1[3]  = dLNormdEta(logfl(_v1) - logfl(_ypred), 1*_std, -_dYdEta, _dSdEta*_std);
				}
			} else {
				if (bBql){
					_dLL2 = lphi((_v1 - (_ypred + _dYdEta*_del)), (_sig+_dSdEta*_del)*_std);
					...
					_dLLdEta1[3]  = dLNormdEta((_v1 - _ypred), _sig*_std, -_dYdEta, _dSdEta*_std);
				}
			}

			_VecAddTo(4, _dLLdEta1, _dLLdEta);
		}
		_dLL[0] += _dLL1;
...
void NewGetPred(int bGenGradients, int* pnObs, double zzY[], double zzIRate[], double _t, const char* _nm, double _v1, bool doActions /*= true*/)
{
	...
	double _dYdEta[4];
	double _dSdEta[4];
	_dYdEta[0] = _dYdEta[1] = _dYdEta[2] = _dYdEta[3] = 0;
	_dSdEta[0] = _dSdEta[1] = _dSdEta[2] = _dSdEta[3] = 0;
    ...
    if (_nm[0]=='C' && strcmp(_nm, "CObs")==0){
		...
		if (bGenGradients){
			...
			_dYdEta[3] = ((V * _A1_nKa) / (V * V)); // Get dY / dnKa
			_dSdEta[3] = 0.0; // Get dS / dnKa
			if (ALLOWLOGTRANSFORM && bEnableLogTransform){
				...
				RecordObservationGradient(_dYdEta[3] / (_ypred + 1e-30), 0, 3);
			} else {
				...
				RecordObservationGradient(_dYdEta[3], _dSdEta[3], 3);
			}
		}
		(*pnObs)++; // count all observations
	...
	)
{
	...
	int _nObs = 0;
	double _dLLLocal[1];
	double _dGradLocal[4];
	memset(_dGrad, 0, sizeof(double)*4);
	_dLLLocal[0] = 0;
    ...
    if (_nm[0]=='C' && strcmp(_nm, "CObs")==0){
        ...
        if (FALSE && ALLOWLOGTRANSFORM && bEnableLogTransform){
            *_pPred = logfl((C + (_eps * sqrt((1.0 + (pow(C, 2.0) * pow((CMultStdev / sigma()), 2.0)))))));
            _eps = _pcb->GetNormal() * _fixef[10]; // get a N(0,stdev)
