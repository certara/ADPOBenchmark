...
void TruncateOmega5(double* _omat){
	...
	_omat[0+0+0*3] = _temp;
	_temp = _mtemp[0+1+0*3];
	_omat[0+1+0*3] = _temp;
	_temp = _mtemp[0+1+1*3];
	_omat[0+1+1*3] = _temp;
	_temp = _mtemp[0+2+0*3];
	_omat[0+2+0*3] = _temp;
	_temp = _mtemp[0+2+1*3];
	_omat[0+2+1*3] = _temp;
...
void   TypXParameters(double* _thvec, double* _omat, double* _pack, int* _ldOmat, int* _nPack){
	...
	}
	{
		_i = 0 + 0;
		_j = 0 + 0;
		_pack[_n++] = sqrt(fabs(_omat[_j * _ldo + _j]) * fabs(_omat[_i * _ldo + _i]));
	}
	{
		_i = 0 + 1;
		_j = 0 + 0;
...
void   GetPackedIndexRanCov(int* _iRan, int* _jRan, int* _index){
	...
	*_index = -1;
	...
	{
		if (*_iRan==1 && *_jRan==0){*_index = _n; return;}
		_n++;
	}
	{
	...
	{
		if (*_iRan==2 && *_jRan==0){*_index = _n; return;}
		_n++;
	}
	{
		if (*_iRan==2 && *_jRan==1){*_index = _n; return;}
		_n++;
	}
	{
...
void Deriv(long* _pN, double* _pt, double zzY[], double zzR[], double zzIRate[]){
	...
	zzR_Aa = ( -(Ka) * Aa);
	zzR_Aa += _Aa_irate;
	zzR_A1 = ((((Ka * Aa) + (K21 * A2)) - (K12 * A1)) - ((Vmax * pow(C, GAMMA)) / (pow(Km, GAMMA) + pow(C, GAMMA))));
	zzR_A1 += _A1_irate;
...
void Jacobian(int* pN, double* pt, double zzY[], double zzIRate[], int* pML, int* pMU, double* _jacobian, int* pNRJ){
	...
	_jacobian[1 + _nrj*0] = tvKa;
		// w.r.t. A1
	_jacobian[1 + _nrj*1] = ( -(K12) - ((((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * ((tvVmax * __temp[2]) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])))) - (((tvVmax * __temp[2]) * pow((A1 / (tvV * __temp[1])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])))) / ((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * (pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)))));
		// w.r.t. A2
...
#define     _A1_nKm       zzY1[2]
#define zzR__A1_nKm       zzR1[2]

#define     _A2_nKm       zzY1[3]
#define zzR__A2_nKm       zzR1[3]

#define     _A1_nV       zzY1[4]
#define zzR__A1_nV       zzR1[4]

#define     _A2_nV       zzY1[5]
#define zzR__A2_nV       zzR1[5]

#define NINTEG_GRADIENT_DERIV 6
...
void GradientDeriv(long* _pN, double* _pt, double zzY[], double zzR[], double zzIRate[])
{
	...
	//[1,4] 0.0
	//[2,4] 0.0
	//[4,4] ( -(K12) - ((((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((tvVmax * exp(nVmax)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) - (((tvVmax * exp(nVmax)) * pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) / ((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)))))  is not constant
	//[5,4] K12
	//[6,4] 0.0
	//[7,4] 0.0
	//[8,4] 0.0
	...
	//[4,6] 0.0
	//[5,6] 0.0
	//[6,6] ( -(K12) - ((((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((tvVmax * exp(nVmax)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) - (((tvVmax * exp(nVmax)) * pow((A1 / (tvV * exp(nKm))), GAMMA)) * ((GAMMA * pow((A1 / (tvV * exp(nKm))), (GAMMA - 1.0))) / (tvV * exp(nKm))))) / ((pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)) * (pow((tvKm * exp(nV)), GAMMA) + pow((A1 / (tvV * exp(nKm))), GAMMA)))))  is not constant
	//[7,6] K12
	//[8,6] 0.0
	//[9,6] 0.0
...
void GradientJacobian(int* pN, double* pt, double zzY[], double zzIRate[], int* pML, int* pMU, double* _gjacobian, int* pNRJ){
	...
	_gjacobian[1 + _nrj*0] = tvKa;
		// w.r.t. A1
	_gjacobian[1 + _nrj*1] = ( -(K12) - ((((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * ((tvVmax * __temp[2]) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])))) - (((tvVmax * __temp[2]) * pow((A1 / (tvV * __temp[1])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])))) / ((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * (pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)))));
		// w.r.t. A2
	...
	// rate terms for _A1_nVmax
		// w.r.t. A1
	_gjacobian[4 + _nrj*1] = ((_A1_nVmax *  -((((((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * (pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA))) * (((((tvVmax * __temp[2]) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1]))) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1]))) + ((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * ((tvVmax * __temp[2]) * (((tvV * __temp[1]) * (GAMMA * (((GAMMA - 1.0) * pow((A1 / (tvV * __temp[1])), ((GAMMA - 1.0) - 1.0))) / (tvV * __temp[1])))) / ((tvV * __temp[1]) * (tvV * __temp[1])))))) - ((((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])) * ((tvVmax * __temp[2]) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])))) + (((tvVmax * __temp[2]) * pow((A1 / (tvV * __temp[1])), GAMMA)) * (((tvV * __temp[1]) * (GAMMA * (((GAMMA - 1.0) * pow((A1 / (tvV * __temp[1])), ((GAMMA - 1.0) - 1.0))) / (tvV * __temp[1])))) / ((tvV * __temp[1]) * (tvV * __temp[1]))))))) - ((((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * ((tvVmax * __temp[2]) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])))) - (((tvVmax * __temp[2]) * pow((A1 / (tvV * __temp[1])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])))) * (((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1]))) + ((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])))))) / (((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * (pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA))) * ((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * (pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA))))))) +  -((((((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * (pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA))) * (((pow((A1 / (tvV * __temp[1])), GAMMA) * (tvVmax * __temp[2])) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1]))) + ((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * ((tvVmax * __temp[2]) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])))))) - (((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * (pow((A1 / (tvV * __temp[1])), GAMMA) * (tvVmax * __temp[2]))) * (((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1]))) + ((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])))))) / (((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * (pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA))) * ((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * (pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)))))));
		// w.r.t. _A1_nVmax
	_gjacobian[4 + _nrj*4] = ( -(K12) - ((((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * ((tvVmax * __temp[2]) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])))) - (((tvVmax * __temp[2]) * pow((A1 / (tvV * __temp[1])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])))) / ((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * (pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)))));
		// w.r.t. _A2_nVmax
	...
	// rate terms for _A1_nKm
		// w.r.t. A1
	_gjacobian[6 + _nrj*1] = ((_A1_nKm *  -((((((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * (pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA))) * (((((tvVmax * __temp[2]) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1]))) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1]))) + ((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * ((tvVmax * __temp[2]) * (((tvV * __temp[1]) * (GAMMA * (((GAMMA - 1.0) * pow((A1 / (tvV * __temp[1])), ((GAMMA - 1.0) - 1.0))) / (tvV * __temp[1])))) / ((tvV * __temp[1]) * (tvV * __temp[1])))))) - ((((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])) * ((tvVmax * __temp[2]) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])))) + (((tvVmax * __temp[2]) * pow((A1 / (tvV * __temp[1])), GAMMA)) * (((tvV * __temp[1]) * (GAMMA * (((GAMMA - 1.0) * pow((A1 / (tvV * __temp[1])), ((GAMMA - 1.0) - 1.0))) / (tvV * __temp[1])))) / ((tvV * __temp[1]) * (tvV * __temp[1]))))))) - ((((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * ((tvVmax * __temp[2]) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])))) - (((tvVmax * __temp[2]) * pow((A1 / (tvV * __temp[1])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])))) * (((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1]))) + ((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])))))) / (((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * (pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA))) * ((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * (pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA))))))) +  -((((((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * (pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA))) * (((((tvVmax * __temp[2]) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) * ( -((A1 * (tvV * __temp[1]))) / ((tvV * __temp[1]) * (tvV * __temp[1]))))) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1]))) + ((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * ((tvVmax * __temp[2]) * ((( -((A1 * (tvV * __temp[1]))) / ((tvV * __temp[1]) * (tvV * __temp[1]))) * (GAMMA * (((GAMMA - 1.0) * pow((A1 / (tvV * __temp[1])), ((GAMMA - 1.0) - 1.0))) / (tvV * __temp[1])))) + ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) * ((((tvV * __temp[1]) * (tvV * __temp[1])) *  -((tvV * __temp[1]))) / (((tvV * __temp[1]) * (tvV * __temp[1])) * ((tvV * __temp[1]) * (tvV * __temp[1]))))))))) - ((((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) * ( -((A1 * (tvV * __temp[1]))) / ((tvV * __temp[1]) * (tvV * __temp[1])))) * ((tvVmax * __temp[2]) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])))) + (((tvVmax * __temp[2]) * pow((A1 / (tvV * __temp[1])), GAMMA)) * ((( -((A1 * (tvV * __temp[1]))) / ((tvV * __temp[1]) * (tvV * __temp[1]))) * (GAMMA * (((GAMMA - 1.0) * pow((A1 / (tvV * __temp[1])), ((GAMMA - 1.0) - 1.0))) / (tvV * __temp[1])))) + ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) * ((((tvV * __temp[1]) * (tvV * __temp[1])) *  -((tvV * __temp[1]))) / (((tvV * __temp[1]) * (tvV * __temp[1])) * ((tvV * __temp[1]) * (tvV * __temp[1])))))))))) - ((((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * ((tvVmax * __temp[2]) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) * ( -((A1 * (tvV * __temp[1]))) / ((tvV * __temp[1]) * (tvV * __temp[1])))))) - (((tvVmax * __temp[2]) * pow((A1 / (tvV * __temp[1])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) * ( -((A1 * (tvV * __temp[1]))) / ((tvV * __temp[1]) * (tvV * __temp[1])))))) * (((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1]))) + ((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])))))) / (((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * (pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA))) * ((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * (pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)))))));
		// w.r.t. _A1_nKm
	_gjacobian[6 + _nrj*6] = ( -(K12) - ((((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * ((tvVmax * __temp[2]) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])))) - (((tvVmax * __temp[2]) * pow((A1 / (tvV * __temp[1])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])))) / ((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * (pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)))));
		// w.r.t. _A2_nKm
	...
	// rate terms for _A1_nV
		// w.r.t. A1
	_gjacobian[8 + _nrj*1] = ((_A1_nV *  -((((((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * (pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA))) * (((((tvVmax * __temp[2]) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1]))) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1]))) + ((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * ((tvVmax * __temp[2]) * (((tvV * __temp[1]) * (GAMMA * (((GAMMA - 1.0) * pow((A1 / (tvV * __temp[1])), ((GAMMA - 1.0) - 1.0))) / (tvV * __temp[1])))) / ((tvV * __temp[1]) * (tvV * __temp[1])))))) - ((((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])) * ((tvVmax * __temp[2]) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])))) + (((tvVmax * __temp[2]) * pow((A1 / (tvV * __temp[1])), GAMMA)) * (((tvV * __temp[1]) * (GAMMA * (((GAMMA - 1.0) * pow((A1 / (tvV * __temp[1])), ((GAMMA - 1.0) - 1.0))) / (tvV * __temp[1])))) / ((tvV * __temp[1]) * (tvV * __temp[1]))))))) - ((((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * ((tvVmax * __temp[2]) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])))) - (((tvVmax * __temp[2]) * pow((A1 / (tvV * __temp[1])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])))) * (((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1]))) + ((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])))))) / (((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * (pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA))) * ((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * (pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA))))))) +  -((((((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * (pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA))) *  -((((GAMMA * pow((tvKm * __temp[0]), (GAMMA - 1.0))) * (tvKm * __temp[0])) * ((tvVmax * __temp[2]) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])))))) - ( -((((tvVmax * __temp[2]) * pow((A1 / (tvV * __temp[1])), GAMMA)) * ((GAMMA * pow((tvKm * __temp[0]), (GAMMA - 1.0))) * (tvKm * __temp[0])))) * (((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1]))) + ((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])))))) / (((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * (pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA))) * ((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * (pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)))))));
		// w.r.t. _A1_nV
	_gjacobian[8 + _nrj*8] = ( -(K12) - ((((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * ((tvVmax * __temp[2]) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])))) - (((tvVmax * __temp[2]) * pow((A1 / (tvV * __temp[1])), GAMMA)) * ((GAMMA * pow((A1 / (tvV * __temp[1])), (GAMMA - 1.0))) / (tvV * __temp[1])))) / ((pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)) * (pow((tvKm * __temp[0]), GAMMA) + pow((A1 / (tvV * __temp[1])), GAMMA)))));
		// w.r.t. _A2_nV
	_gjacobian[8 + _nrj*9] = K21;
